...
build_spinor() {
    rm -f /tmp/spi.img
    if [[ -f "$SCRIPT_DIR/u-boot-rockchip-spi.bin" ]]
    then
        echo "Building Upstream SPI U-Boot..."
        truncate -s 4M /tmp/spi.img
        dd conv=notrunc,fsync if="$SCRIPT_DIR/u-boot-rockchip-spi.bin" of=/tmp/spi.img bs=512
    elif [[ -f "$SCRIPT_DIR/idbloader-spi_spl.img" ]] && [[ -f "$SCRIPT_DIR/u-boot.itb" ]]
    then
